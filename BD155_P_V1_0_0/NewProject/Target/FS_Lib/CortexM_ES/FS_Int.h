...
#define FILE_BUFFER_FLAGS_VALID       (1 << 7)    // The most significant bit of the file buffer flags indicates if the other flags are valid.

#define FS_VERSION2STRING_HELPER(x)   #x          //lint !e9024 hash in macro.
...
*    The first 3 entries of the structure have to be identical
*    to the entries in FS_INFO.Info
*/
struct FS_FAT_INFO {
...
*       FS_JOURNAL_DATA
*/
typedef struct FS_JOURNAL_DATA {
...
typedef int         FS_CB_REMOVEDIR           (FS_VOLUME  * pVolume,   const char * sDirName);
typedef int         FS_CB_CREATEDIR           (FS_VOLUME  * pVolume,   const char * sDirName);
typedef int         FS_CB_DELETEDIR           (FS_VOLUME  * pVolume,   const char * sDirName, int MaxRecursionLevel);
...
*       FS_INT_DATA_FAT
*/
typedef struct FS_INT_DATA_FAT {
...
*       FS_INT_DATA_EFS
*/
typedef struct FS_INT_DATA_EFS {
...
*/
struct FS_SB {
  U32            SectorNo;        // Index of the sector to be accessed.
...
*/
typedef struct CACHE_ALL_BLOCK_INFO {
  U32 SectorNo;
} CACHE_ALL_BLOCK_INFO;
...
*/
typedef struct CACHE_MAN_BLOCK_INFO {
  U32 SectorNo;
} CACHE_MAN_BLOCK_INFO;
...
*/
typedef struct CACHE_RW_BLOCK_INFO {
  U32 SectorNo;
  U32 IsDirty;
} CACHE_RW_BLOCK_INFO;
...
*/
typedef struct CACHE_RW_QUOTA_BLOCK_INFO {
  U32 SectorNo;
  U32 IsDirty;
} CACHE_RW_QUOTA_BLOCK_INFO;
...
*  FS internal structure. Once instance per cache. Size needs to be a multiple of 4.
*/
typedef struct CACHE_RW_QUOTA_DATA {
  U32 NumSectors;
  U32 SectorSize;
  U8  aCacheMode[FS_SECTOR_TYPE_COUNT];
#if FS_SECTOR_TYPE_COUNT % 4
...
*/
typedef struct CACHE_MULTI_WAY_BLOCK_INFO {
...
*/
typedef struct CACHE_MULTI_WAY_DATA {
...
*/
typedef struct FS_WRBUF_STAT_COUNTERS {
...
int          FS__GetFileInfo         (const char * pName, FS_FILE_INFO * pInfo);
...
typedef struct FS_GLOBAL {
  ...
  U8              FileBufferFlags;
#endif
#if (FS_SUPPORT_EFS && FS_EFS_SUPPORT_DIRENTRY_BUFFERS)
...
#define FS_IOCTL(    pDevice, Cmd, Aux, pBuffer)                FS_LB_Ioctl(pDevice, Cmd, Aux, pBuffer)
  ...
  #define FS_DELETEDIR(pVolume, s, MaxRecursionLevel)                 FS_FAT_DeleteDir(pVolume, s, MaxRecursionLevel)
  ...
  #define FS_DELETEDIR(pVolume, s, MaxRecursionLevel)                 FS_EFS_DeleteDir(pVolume, s, MaxRecursionLevel)
  ...
  #define FS_DELETEDIR(pVolume, s, MaxRecursionLevel)                 FS_MAP_DeleteDir(pVolume, s, MaxRecursionLevel)
  ...
  #define FS_DELETEDIR(pVolume, sDirName, MaxRecursionLevel)          0
...
int         FS_MAP_CheckDisk           (FS_VOLUME  * pVolume,   const FS_DISK_INFO * pDiskInfo, void * pBuffer, U32 BufferSize, int MaxRecursionLevel, FS_CHECKDISK_ON_ERROR_CALLBACK * pfOnError);
...
char *      FS__CLIB_strchr  (const char * s, char c);
